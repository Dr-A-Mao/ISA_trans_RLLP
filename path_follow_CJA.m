...
%
function out = path_follow_CJA(in,P)
  %% 根据dubins路径给出的参考状态量
  % 同时也是JGCD的
  NN = 0;
  flag        = in(1+NN);
  ...
  switch flag
      case 1 % follow straight line path specified by c_path, psi_path and gam_path
          ...
          % 制导率
          if strcmp(P.path_type, 'lissajous_CJA')
          ...
          d_hat = [ P.wind_n; P.wind_e; P.wind_d]; % 考虑加负号

          [phi_c,gamma_c] = CJA_guidance_law(pn,pe,h, ...
                                             Va,psi,d_hat,P, ...
                                             curvetype,C,l);
  end
