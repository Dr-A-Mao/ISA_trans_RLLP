function [x_p,y_p,z_p] = get_track_point(P,x_r,y_r,z_r,Vg_r,gamma_r,chi_r)
        % 得到关键参量, x_r: north, y_r: east
        path_x = P.waypoints(:,2); % north
        path_y = P.waypoints(:,1); % east
        path_z = P.waypoints(:,3); 
        % 路径参量
        path_eta_lat = atan2(path_y - y_r,path_x - x_r) - chi_r;
        path_eta_lon = atan2(path_z - z_r, sqrt((path_x - x_r).^2 + (path_y - y_r).^2)) - gamma_r;
        path_index =  find((abs(path_eta_lat) <= pi/2 * 1) & (abs(path_eta_lon) <= pi/2));
