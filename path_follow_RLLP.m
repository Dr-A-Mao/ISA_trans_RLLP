...
%
function out = path_follow_RLLP(in,P)
  %% 根据dubins路径给出的参考状态量
  NN = 0;
  flag        = in(1+NN);
  Va_d        = in(2+NN);
  ...
  switch flag,
      case 1, % follow straight line path specified by c_path, psi_path and gam_path
          ...
          % ensure chi_c_dot,gamma_c_dot = 0
          chi_c_dot = 0;
          gamma_c_dot = 0;
...
%          disp(['t:',num2str(t),',chi_c:',num2str(chi_c),',gamma_c:',num2str(gamma_c)]);
          % obtain accelerations 
          [a_yc,a_zc] = guidance_law(Va_c,phi,chi,gamma,...
                                    chi_c,gamma_c,...
          ...
          % obtain gamma_c
          gamma_c = obtain_gamma_c(n_lf_c,n_lf,t,P);

  end
  % calculate the I_f
  if P.f_type == 1
      I_f = max(P.k_chi,P.k_gamma) / ( 2 * min(P.k_chi,P.k_gamma)^2 );
  elseif P.f_type == 2
      I_f = pi* max(P.k_chi,P.k_gamma) / ( 4 * min(P.k_chi,P.k_gamma)^2 );
  elseif P.f_type == 3
      I_f = max(P.k_chi,P.k_gamma) / ( 2 * min(P.k_chi,P.k_gamma)^2 );
  elseif P.f_type == 4
      L_c = min(P.k_chi * exp(-pi/2 * P.k_chi), P.k_gamma * exp(-pi/2 * P.k_gamma));
      I_f = max(P.k_chi,P.k_gamma) / (2 * min(P.k_chi,P.k_gamma) * L_c);
  end

%   if abs( t - 109) <= 0.1
%       disp('请注意！！！');
%       disp('此处中断寻找问题原因！');
%       disp('待跟踪航路点：[',num2str(pn_c),',',num2str(pe_c),',',num2str(h_c),']');
%   end
