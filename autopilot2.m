...
%    bz       = uu(19+NN); % z-gyro bias
    ...
    n_lf_c   = uu(2+NN);  % commanded flight path angle (rad)
...
%        delta_e = pitch_hold(theta_c, theta, q, 1, P);
        delta_e = n_lf_hold(n_lf_c, n_lf, 1, P);
...
%             theta_c = theta_max;
%         elseif theta_c < -theta_max
%             theta_c = -theta_max;
%         end       
%       delta_e = pitch_hold(theta_c, theta, q, 0, P);

        delta_e = n_lf_hold(n_lf_c, n_lf, 0, P);
...
function delta_e = n_lf_hold(n_lf_c, n_lf, flag, P)
  persistent integrator;
  persistent error_d1;
  % initialize persistent variables at beginning of simulation
  if flag==1,
  ...
  % integral term
  ui = P.n_lf_ki * integrator;
  
  
  % implement PI - control
  delta_e = sat(up + ui, 45*pi/180, -45*pi/180);
  
  % implement integrator anti-windup
  if P.n_lf_ki~=0,
    delta_e_unsat = up + ui;
    k_antiwindup = P.Ts/P.n_lf_ki;
    integrator = integrator + k_antiwindup*(delta_e-delta_e_unsat);
  end
