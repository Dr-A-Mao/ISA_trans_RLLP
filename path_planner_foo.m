...
function out = path_planner_foo(in,P,map)
  ...
  switch 6,
      ...
      case 5,
           wpp = [...
                  0,   0,   -100,    90*pi/180,     P.Va0;...
                  0, 200,   -100,  0*pi/180,     P.Va0;... 
                  200, 200,   -100,  -90*pi/180,     P.Va0;... 
                  200, 0,   -100,  30*pi/180,     P.Va0;...
                  0, 0,   -100,  30*pi/180,     P.Va0;...
                  0, 500,   -100,  30*pi/180,     P.Va0;... % 必须要加一个不知道为什么要加的行
                  ];
           num_waypoints = size(wpp,1) - 1;
      case 6,
          % 获取参考路径
          path_ref = P.path_ref;
          num_path_points = size(path_ref,1);
          path_chi_Va = zeros(num_path_points,2);
          for k = 1:num_path_points - 1
              path_chi_Va(k,1) = atan2(path_ref(k+1,2) - path_ref(k,2),...
                                       path_ref(k+1,1) - path_ref(k,1));
              path_chi_Va(k,2) = P.Va0;
          end
          path_chi_Va(num_path_points,:) = path_chi_Va(num_path_points-1,:);
          wpp = [path_ref,path_chi_Va];
  ...
  %% 下面这个地方很容易出bug
  out = [num_waypoints; reshape(wpp', 5*P.size_waypoint_array, 1)];  %% 这里有问题！！！
