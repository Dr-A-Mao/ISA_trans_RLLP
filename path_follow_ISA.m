...
%
function out = path_follow_ISA(in,P)
  %% 根据dubins路径给出的参考状态量
  % 同时也是JGCD的
  NN = 0;
  flag        = in(1+NN);
  ...
  switch flag
      case 1 % follow straight line path specified by c_path, psi_path and gam_path
          ...
          % 关键的对比组对于跟踪点方式
          [pn_c,pe_c,h_c,ref_p_index] = select_ref_point(pn,pe,h,...
                                          Va,chi,gamma,...
                                          P.path_ref(:,1),...
                                          P.path_ref(:,2),...
                                          -P.path_ref(:,3));
          % 找到当前航路点对应的索引,从而确定曲率
          if ref_p_index == 1 || ref_p_index == length(P.path_ref(:,1))
              
              kau = 0;

              psi_tilde = 0;
          else
             ref_p_index_before = ref_p_index - 1;
             ref_p_index_after = ref_p_index + 1;

             P_i_before = [P.path_ref(ref_p_index_before,1),...
                           P.path_ref(ref_p_index_before,2)];

             P_i_after = [P.path_ref(ref_p_index_after,1),...
                           P.path_ref(ref_p_index_after,2)];

             P_i_middle = [pn_c,pe_c]; 

             v1 = P_i_middle - P_i_before;
             v2 = P_i_after - P_i_middle;

             theta = acos(sum(v1.*v2)/(norm(v1) * norm(v2)));
             L_bar = 0.5 * (norm(v1) + norm(v2));
             
             kau = theta/L_bar;

             psi_tilde = atan2(pe_c - P_i_before(2), pn_c - P_i_before(1));

          end

         
          e_s = (pn - pn_c) * cos(psi_tilde) + (pe - pe_c) * sin(psi_tilde);

          e_d = -(pn - pn_c) * sin(psi_tilde) + (pe - pe_c) * cos(psi_tilde);

          psi = atan2( pe_c - pe, pn_c - pn); % LOS angle theta
            
          psi_bar = psi - psi_tilde;
          ...
          s = psi_bar + k_pi * tanh(k_d * e_d);

          l_dot = k_s * e_s + Va * cos(psi_bar);

          delta = Va * sin(psi_bar) - kau * e_s * l_dot;

          w = -k_w * s + kau * l_dot - k_pi * k_d * (sech(k_d * e_d))^2 * delta ;

          psiTilde = psiTilde + w * (t - t_old);

          if psiTilde > 2*pi
              psiTilde = psiTilde - 2*pi;
          elseif psiTilde < -2*pi
              psiTilde = psiTilde + 2*pi;
          end

          disp(['psiTilde=',num2str(psi_tilde)]);

          % 根据psi_c得到phi_c, phi_c comes from PID on psiTilde (HACK)
          k_phi = 0.3;
...
function [x_p,y_p,z_p,p_index] = select_ref_point(x_r,y_r,z_r,Vg_r,chi_r,gamma_r,...
